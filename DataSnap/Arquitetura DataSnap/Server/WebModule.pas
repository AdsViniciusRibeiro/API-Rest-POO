...
uses
  ...
  DataSnap.DSAuth,
  Datasnap.DSProxyJavaScript, IPPeerServer, Datasnap.DSMetadata, Datasnap.DSServerMetadata, Datasnap.DSClientMetadata, Datasnap.DSCommonServer,
...
type
  TWebModule1 = class(TWebModule)
    ...
    procedure DSRESTWebDispatcher1FormatResult(Sender: TObject; var ResultVal: TJSONValue; const Command: TDBXCommand; var Handled: Boolean);
  private
...
uses
  System.Generics.Collections, ServerMethods, ServerContainer, Web.WebReq;

procedure TWebModule1.DSRESTWebDispatcher1FormatResult(Sender: TObject; var ResultVal: TJSONValue; const Command: TDBXCommand; var Handled: Boolean);
var
  AResultVal: TJSONValue;
begin
  AResultVal := ResultVal;
  try
    ResultVal := TJSONArray(AResultVal).Remove(0);
  finally
    AResultVal.Free;
  end;
...
begin
  if SameText(TagString, 'urlpath') then
    ...
    ReplaceText := ServerMethods.TServerMethods.ClassName
